...
void Spell::finish(bool ok)
{
    ...
    // remove bladestorm aura by disarm
    if (m_spellInfo->Mechanic == MECHANIC_DISARM && m_targets.GetUnitTarget() && m_targets.GetUnitTarget()->ToPlayer() && m_targets.GetUnitTarget()->ToPlayer()->HasAura(46924))
        m_targets.GetUnitTarget()->ToPlayer()->RemoveAura(46924);

    // Stop Attack for some spells
    if (m_spellInfo->Attributes & SPELL_ATTR0_STOP_ATTACK_TARGET)
        m_caster->AttackStop();

    if (m_spellInfo->Id == 58984 && m_caster->GetTypeId() == TYPEID_PLAYER)
    {
        Player * _pl = m_caster->ToPlayer();
        _pl->CombatStop();
        _pl->getHostileRefManager().deleteReferences();
    }
}
...
SpellCastResult Spell::CheckPetCast(Unit* target)
{
    ...
    if (!target && m_spellInfo->Id == 58913)
    {
